...
def report_datasets(start_date, end_date, user):
    ...
    access = DataSet.ACCESS_NGEET
    q_perm = Q(status__gte=status, access_level__gte=access)
    q_created = Q()
    ...
    # Build sub queries
    q_approved = Q(Q(status=DataSet.STATUS_APPROVED,
                     modified_date__gte=start_date,
                     modified_date__lte=end_date) & q_perm)
